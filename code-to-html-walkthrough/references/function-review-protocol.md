...
只区分“便宜快速档”和“较贵强能力档”，不绑定供应商、具体型号、版本或固定的推理强度枚举。每次运行依据工具暴露的可用模型、能力说明及已知成本信息选择对应档位；不能只凭名称、版本新旧或价格推断能力。

| 工作 | 默认档位 / 推理投入 | 边界 |
| --- | --- | --- |
| 清单、符号、调用点等有明确规则的事实提取 | 便宜快速档 / 较低投入 | 主代理仍执行两种手段的清单核对 |
| 普通函数、语义块、逐行解释草稿 | 便宜快速档 / 适中投入 | 完整函数上下文；明确的短 wrapper 可降低投入 |
| 普通函数独立复查 | 较贵强能力档 / 较高投入 | 另一个代理重新读取源码，不只检查 JSON 格式 |
| 复杂函数、全局通信/内存关系的分析与独立复核 | 较贵强能力档 / 较高投入，必要时提高 | 包括复杂模板/宏、内联汇编、原子与异步同步、跨 Rank 协议、allocator/生命周期及 `memory_model` |

具体模型 ID 和推理强度参数在派发时映射为当前工具支持的值，不把档位名称作为模型 ID。型号更新、退休或不可用时，重新选择已确认可用的同档替代并记录，不依赖历史型号继续存在。没有合适的廉价模型或工具不支持选模时，使用已确认足以完成任务的可用配置，并说明成本优化未启用；成本信息不足时标记未确认，不得假称已降档或节省费用。没有足够能力的 reviewer 或受预算限制无法升级时，保留 `DRAFTED/REWORK` 并报告缺口，不得降低门禁。模型名称不同不等于代理独立，同一代理换模型仍不能复查自己的草稿；不同代理可以使用同一个足够强的模型。

### 3.2 最小充分上下文

- 创建子代理时，工具支持选模或推理强度控制才显式传对应的 `model`、`reasoning_effort`，且值必须来自当前工具支持的选项；不支持的可选参数省略并记录限制。支持时使用 `fork_turns="none"`；不要在全历史继承模式下请求模型覆盖，也不要让固定模型的角色覆盖预期路由。
...
在中间产物旁保留 `dispatch-ledger.json`，记录批次/函数 ID、代理 ID、草稿或复查角色、所选档位及依据、请求模型/推理强度、实际模型（可确认时）、工具限制或替代原因、升级原因与返工次数；该调度记录独立于渲染用分析 JSON。
