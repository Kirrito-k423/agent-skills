...
区间吞吐复用这些完整观测：同一个 end 的新增首次就绪块合并，保留零增量测量点；`copiedOrZero` 对应复制事件不能再次计入到达。首测计数单独保存；本机复制默认排除。B/包、μs/cycle 是显示会话的用户配置，不属于已验证硬件参数，也不写回原始模型。具体规则见 [区间带宽与组成](bandwidth.md)。
