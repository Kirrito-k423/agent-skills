...
生成图、仓库图、上传图必须是同一个图像文件的连续副本：

- 使用 `image_gen` 后，先定位工具实际保存的原始 PNG，再复制到 `assets/<article-slug>-illustrations/`。
- 不要因为找不到生成文件就用 Pillow、SVG、手绘脚本或其他渲染方式重画一张来冒充生成图。
- 上传前用 `shasum -a 256` 或等价方式确认仓库图与原始生成图一致。
- 上传时传入仓库中的最终图片绝对路径；Markdown 只能引用该文件上传得到的 URL。
- 如果原始生成文件无法定位，重新生成图片并重复检查，不要上传风格不同的替代图。
